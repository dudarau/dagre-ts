...
describe('normalize', function () {
  ...
  describe('run', function () {
    ...
    it('splits a two layer edge into two segments', function () {
      ...
      expect((g.successors('a') as string[]).length).toBe(1);
      const successor = (g.successors('a') as string[])[0];
      ...
      expect((g.graph() as any).dummyChains.length).toBe(1);
      expect((g.graph() as any).dummyChains[0]).toBe(successor);
    });
    ...
    it('assigns width and height from the edge for the node on labelRank', function () {
      ...
      const labelV = (g.successors((g.successors('a') as string[])[0]) as string[])[0];
    ...
    it('preserves the weight for the edge', function () {
      ...
      expect((g.successors('a') as string[]).length).toBe(1);
      expect(g.edge('a', (g.successors('a') as string[])[0]).weight).toBe(2);
    });
  });
  ...
  describe('undo', function () {
    ...
    it("collects assigned coordinates into the 'points' attribute", function () {
      ...
      const dummyLabel = g.node((g.neighbors('a') as string[])[0]);
      dummyLabel.x = 5;
      dummyLabel.y = 10;
    ...
    it("merges assigned coordinates into the 'points' attribute", function () {
      ...
      const aSucLabel = g.node((g.neighbors('a') as string[])[0]);
      aSucLabel.x = 5;
      aSucLabel.y = 10;

      const midLabel = g.node((g.successors((g.successors('a') as string[])[0]) as string[])[0]);
      midLabel.x = 20;
      midLabel.y = 25;

      const bPredLabel = g.node((g.neighbors('b') as string[])[0]);
      bPredLabel.x = 100;
      bPredLabel.y = 200;
    ...
    it('sets coords and dims for the label, if the edge has one', function () {
      ...
      const labelNode = g.node((g.successors('a') as string[])[0]);
      labelNode.x = 50;
      labelNode.y = 60;
      labelNode.width = 20;
      labelNode.height = 10;
      ...
      const { x, y, width, height } = g.edge('a', 'b');
    ...
    it('sets coords and dims for the label, if the long edge has one', function () {
      ...
      const labelNode = g.node((g.successors((g.successors('a') as string[])[0]) as string[])[0]);
      labelNode.x = 50;
      labelNode.y = 60;
      labelNode.width = 20;
      labelNode.height = 10;
    ...
    it('restores multi-edges', function () {
      ...
      const outEdges = g.outEdges('a') as Edge[]; //, 'name';
